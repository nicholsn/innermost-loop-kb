...
Owner review follow-up: theme, reporting-date, and measure controls now update the views automatically. Growth points expose summary tooltips on pointer hover and keyboard focus, including new records, cumulative records in range, the monthly denominator, and coverage share. Escape dismisses the tooltip. Browser checks verified live measure/theme/date updates, focus summaries, and dismissal while preserving the reviewer's selected comparison.
