...
function drawGrowth(){
 ...
 result.series.forEach((s:any,idx:number)=>{
  ...
  s.points.forEach((p:any,i:number)=>{if(p[mode]===null)return;const circle=svgNode('circle',{cx:x(i),cy:y(p[mode]),r:5,fill:palette[idx],tabindex:0,role:'button','aria-label':`${themes.get(s.id).title}, ${p.month}: ${Number(p[mode]).toFixed(mode==='share'?1:0)}${mode==='share'?' percent':''}. Inspect new records this month.`});circle.append(svgNode('title',{},`${themes.get(s.id).title}: ${p.month} · ${p.count} new records`));circle.addEventListener('click',()=>inspect(p.month,s.id));circle.addEventListener('keydown',(e:any)=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();inspect(p.month,s.id);}});svg.append(circle);});
...
form.addEventListener('submit',e=>{e.preventDefault();actor='';shown=20;render(true);});
...
el('analytics-reset').addEventListener('click',()=>{slots.forEach((s,i)=>s.value=data.themes[i]?.id||'');from.value=data.min;to.value=data.max;metric.value='cumulative';month.value=theme.value=actor='';shown=20;render(true);});
