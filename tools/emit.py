...
def _round_trips_as_str(s: str, *, flow: bool = False) -> bool:
    """True when YAML reads ``s`` back as the same string, not an int/date/bool.

    Tested in the context it will be written in: a plain scalar that is fine
    as a block value can still break an inline ``{ k: v }`` mapping - a title
    ending in ``?`` reads as a complex-key indicator there.
    """
    try:
        if flow:
            return yaml.safe_load("v: { k: " + s + " }")["v"]["k"] == s
...
def _yaml_scalar(v: str, *, flow: bool = False) -> str:
    ...
    if flow and s and any(c in s for c in ",{}[]"):
        risky = True
    if not risky and s and not _round_trips_as_str(s, flow=flow):
