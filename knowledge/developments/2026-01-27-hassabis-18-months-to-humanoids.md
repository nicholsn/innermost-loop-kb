---
type: Development
title: Hassabis puts humanoid robotics 18 months out
claim: Demis Hassabis predicted DeepMind is only eighteen months from solving humanoid robotics, while Washington State moved to require 3D printers to detect and block gun manufacture.
domain: robotics
reported_in:
  - https://nicholsn.github.io/innermost-loop-kb/issues/2026-01-27
actor:
  - https://nicholsn.github.io/innermost-loop-kb/organizations/google-deepmind
  - https://nicholsn.github.io/innermost-loop-kb/people/demis-hassabis
  - https://nicholsn.github.io/innermost-loop-kb/organizations/washington-state
evidences:
  - https://nicholsn.github.io/innermost-loop-kb/themes/physical-recursion
  - https://nicholsn.github.io/innermost-loop-kb/themes/legislating-the-shift
score: 18 months
tags:
  - "development"
  - "2026-01-27"
generated: { by: process:iml-emit, at: 2026-01-27T00:00:00Z }
sources:
  - { id: iml-2026-01-27, resource: https://theinnermostloop.substack.com/p/welcome-to-january-27-2026, title: "Welcome to January 27, 2026", author: human:alex-wissner-gross, last_modified: 2026-01-27 }
---


