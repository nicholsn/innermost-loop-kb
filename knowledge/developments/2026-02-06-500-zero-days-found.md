---
type: Development
title: A model finds 500 zero-days, some decades old
claim: Opus 4.6 discovered 500 zero-day vulnerabilities in open source codebases, including some that had gone undetected for decades, while Anthropic launched Agent Teams for multi-agent coordination and added server-side compaction.
domain: agents
reported_in:
  - https://nicholsn.github.io/innermost-loop-kb/issues/2026-02-06
actor:
  - https://nicholsn.github.io/innermost-loop-kb/organizations/anthropic
evidences:
  - https://nicholsn.github.io/innermost-loop-kb/themes/automated-science
  - https://nicholsn.github.io/innermost-loop-kb/themes/agents-on-the-org-chart
score: 500 zero-days
supersedes:
  - https://nicholsn.github.io/innermost-loop-kb/developments/2026-02-03-codex-builds-itself
tags:
  - "development"
  - "2026-02-06"
generated: { by: process:iml-emit, at: 2026-02-06T00:00:00Z }
sources:
  - { id: iml-2026-02-06, resource: https://theinnermostloop.substack.com/p/welcome-to-february-6-2026, title: "Welcome to February 6, 2026", author: human:alex-wissner-gross, last_modified: 2026-02-06 }
---


