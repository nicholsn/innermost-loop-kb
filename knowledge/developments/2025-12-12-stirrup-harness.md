---
type: Development
title: Artificial Analysis releases the Stirrup harness
claim: Artificial Analysis released Stirrup, a harness that improves baseline model performance on economic tasks purely by framing problems better.
domain: agents
reported_in:
  - https://nicholsn.github.io/innermost-loop-kb/issues/2025-12-12
actor:
  - https://nicholsn.github.io/innermost-loop-kb/organizations/artificial-analysis
about:
  - https://nicholsn.github.io/innermost-loop-kb/systems/stirrup
evidences:
  - https://nicholsn.github.io/innermost-loop-kb/themes/scaffolding-over-weights
tags:
  - "development"
  - "2025-12-12"
generated: { by: process:iml-emit, at: 2025-12-12T00:00:00Z }
sources:
  - { id: iml-2025-12-12, resource: https://theinnermostloop.substack.com/p/welcome-to-december-12-2025, title: "Welcome to December 12, 2025", author: human:alex-wissner-gross, last_modified: 2025-12-12 }
---


