---
type: Development
title: Altman promises a model 100x more capable and cheaper
claim: Sam Altman promised a model a hundred times more capable, faster and cheaper than current frontier systems, with OpenAI aiming to compress 25 years of science into five and already handling 8.4 million weekly messages on advanced math and physics.
domain: models
reported_in:
  - https://nicholsn.github.io/innermost-loop-kb/issues/2026-01-27
actor:
  - https://nicholsn.github.io/innermost-loop-kb/organizations/openai
  - https://nicholsn.github.io/innermost-loop-kb/people/sam-altman
evidences:
  - https://nicholsn.github.io/innermost-loop-kb/themes/reasoning-price-deflation
  - https://nicholsn.github.io/innermost-loop-kb/themes/automated-science
score: 100x / 25 years in 5
tags:
  - "development"
  - "2026-01-27"
generated: { by: process:iml-emit, at: 2026-01-27T00:00:00Z }
sources:
  - { id: iml-2026-01-27, resource: https://theinnermostloop.substack.com/p/welcome-to-january-27-2026, title: "Welcome to January 27, 2026", author: human:alex-wissner-gross, last_modified: 2026-01-27 }
---


