---
type: Development
title: Microsoft sets a goal of removing all C and C++ by 2030
claim: Microsoft set a North Star goal to eliminate every line of C and C++ from its codebase by 2030, targeting a velocity where one engineer rewrites a million lines a month.
domain: society
reported_in:
  - https://nicholsn.github.io/innermost-loop-kb/issues/2025-12-23
actor:
  - https://nicholsn.github.io/innermost-loop-kb/organizations/microsoft
evidences:
  - https://nicholsn.github.io/innermost-loop-kb/themes/engineer-as-supervisor
score: 1M lines/engineer/month
tags:
  - "development"
  - "2025-12-23"
generated: { by: process:iml-emit, at: "2025-12-23T00:00:00Z" }
sources:
  - { id: iml-2025-12-23, resource: https://theinnermostloop.substack.com/p/welcome-to-december-23-2025, title: "Welcome to December 23, 2025", author: human:alex-wissner-gross, last_modified: "2025-12-23" }
---


