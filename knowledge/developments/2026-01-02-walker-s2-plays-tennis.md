---
type: Development
title: A humanoid plays competitive tennis
claim: UBTECH's Walker S2 is playing competitive tennis, and Figure predicts humanoids will run unsupervised multi-day household tasks within the year.
domain: robotics
reported_in:
  - https://nicholsn.github.io/innermost-loop-kb/issues/2026-01-02
actor:
  - https://nicholsn.github.io/innermost-loop-kb/organizations/ubtech
  - https://nicholsn.github.io/innermost-loop-kb/organizations/figure
evidences:
  - https://nicholsn.github.io/innermost-loop-kb/themes/autonomy-clock-speed
  - https://nicholsn.github.io/innermost-loop-kb/themes/physical-recursion
tags:
  - "development"
  - "2026-01-02"
generated: { by: process:iml-emit, at: 2026-01-02T00:00:00Z }
sources:
  - { id: iml-2026-01-02, resource: https://theinnermostloop.substack.com/p/welcome-to-january-2-2026, title: "Welcome to January 2, 2026", author: human:alex-wissner-gross, last_modified: 2026-01-02 }
---


