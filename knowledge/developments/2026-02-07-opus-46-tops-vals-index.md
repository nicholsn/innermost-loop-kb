---
type: Development
title: Opus 4.6 takes the top index spot as markets price the next month
claim: Claude Opus 4.6 took first on the Vals Index and the Code and Text Arenas while statistically tying GPT-5.2-xhigh across FrontierMath Tiers 1 to 4, with prediction markets giving Anthropic 67% odds of the best model by month's end.
domain: benchmarks
reported_in:
  - https://nicholsn.github.io/innermost-loop-kb/issues/2026-02-07
actor:
  - https://nicholsn.github.io/innermost-loop-kb/organizations/anthropic
about:
  - https://nicholsn.github.io/innermost-loop-kb/benchmarks/vals-index
evidences:
  - https://nicholsn.github.io/innermost-loop-kb/themes/benchmark-saturation
  - https://nicholsn.github.io/innermost-loop-kb/themes/spiky-frontier
score: 67% odds
supersedes:
  - https://nicholsn.github.io/innermost-loop-kb/developments/2026-02-06-record-falls-in-thirty-minutes
tags:
  - "development"
  - "2026-02-07"
generated: { by: process:iml-emit, at: 2026-02-07T00:00:00Z }
sources:
  - { id: iml-2026-02-07, resource: https://theinnermostloop.substack.com/p/welcome-to-february-7-2026, title: "Welcome to February 7, 2026", author: human:alex-wissner-gross, last_modified: 2026-02-07 }
---


